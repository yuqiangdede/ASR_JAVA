...
public class AudioDecodeService {
    ...
    private float[] extractSamples(Frame frame) {
        ...
        if (sampleBuffer instanceof FloatBuffer) {
            FloatBuffer duplicate = ((FloatBuffer) sampleBuffer).duplicate();
            duplicate.rewind();
        ...
        if (sampleBuffer instanceof ShortBuffer) {
            ShortBuffer duplicate = ((ShortBuffer) sampleBuffer).duplicate();
            duplicate.rewind();
            float[] samples = new float[duplicate.remaining()];
